...
插件会对每个核心比例重新计算 CV、HE、SH、EN、AN 和 EE。原脚本中的 `21%` 不是通用阈值，因此不再写死：
...
- `自动`：先使用快速随机爬山比较候选规模，再仅对推荐规模使用并行回火精细复核。
- `快速`：仅执行随机爬山，适合探索规模和检查输入。
- `精细`：仅用于固定比例或固定材料数，使用官方并行回火算法。

默认不再强制运行巨大的固定步数。快速模式按“单次最长时间 + 连续无改进时间”停止，精细模式同样使用时间与收敛条件，避免对整段比例执行数亿次无效邻域搜索。运行过程中会报告当前阶段、核心集规模、重复次数、真实完成任务数和持续时间。
...
- `随机种子`用于重复得到相同搜索起点。
- `精细重复次数`会使用不同派生种子并保留目标表现最好的核心集。
- 结果中的`材料重合稳定性`是最优集合与各次重复集合之间 Jaccard 重合度的平均值。
- `必须保留`可锁定关键材料。
- `排除`可过滤失活、重复或不可用材料。
- 精细模式适合固定规模的最终复核；快速模式适合比例扫描。

## 导出

Excel 报告包括：分析概览、六类指标的完整比例扫描、当前核心材料、材料约束和完整 JSON 结果。报告保留距离方法、目标权重、Core Hunter 版本、随机种子、时间控制、重复次数、材料重合稳定性和运行耗时。
